...
console.log(`The first character ${message} of  is ${message.charAt(0)}`)
// Log the last character of message using length like `The last character of [message] is [character]`
console.log(`The last character ${message} of  is ${message.charAt(message.length - 1)}`)
...
console.log(message.endsWith('answers'));
...
let newMessage = console.log(message.replace('are','can\'t'));
...
newMessage = console.log(message.replace('thinking','decide'));
...
let messageArray = console.log(message.split(""," "));
